...
(function initLoginPage() {
  ...
  loginBtn.addEventListener("click", () => {
    ...
    window.location.href = "prebid.html";
  });
})();
...
(function initAdminPage() {
  ...
  function startItem(itemNo) {
    ...
    // نظّف قائمة المزايدات الحية للعرض (اختياري)
    // نخليها تراكمية حسب القطعة
  ...
  function resetState() {
    ...
    alert("تم إيقاف/إعادة ضبط القطعة ✅");
  ...
  function renderStatus() {
    ...
    stPrice.textContent = formatPrice(state.currentPrice);
    stLast.textContent = state.lastPaddle ? `#${state.lastPaddle}` : "—";
    const remaining = Math.max(0, HOA.DURATION_SEC - Math.floor((now() - state.startedAt) / 1000));
    stTimer.textContent = mmss(remaining) + (state.status === "SOLD" ? " (SOLD)" : "");
  }
